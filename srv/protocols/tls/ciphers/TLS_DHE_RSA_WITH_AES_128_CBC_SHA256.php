...
class TLS_DHE_RSA_WITH_AES_128_CBC_SHA256
{
	const CIPHER_CODE = [0x00,0x67];
	const CIPHER_SIGNATURE_ALGO = [0x04,0x01];
    ...
    {
        $signature = "";
        openssl_sign($clientRandom.$serverRandom.$DHParams, $signature, $privateKey,"sha256WithRSAEncryption");
        return $signature;
    }
    ...
    public function generateEncryptionKeys($master_secret, $client_random, $server_random) {
        ...
        $keys = ['client'=>[],'server'=>[]];
		$keys['client']['mac-key'] = substr($key_buffer, 0, 32);
        $keys['server']['mac-key'] = substr($key_buffer, 32, 32);
        $keys['client']['write-key'] = substr($key_buffer, 64, 16);
        $keys['server']['write-key'] = substr($key_buffer, 80, 16);
        $keys['client']['iv-key'] = substr($key_buffer, 96, 16);
        $keys['server']['iv-key'] = substr($key_buffer, 112, 16);
        return $keys;
    }
